...
import { getStudentsByClass, MOCK_STUDENTS } from "@/lib/mock-data/documents";
...
export async function POST(req: Request) {
  ...
  const zipBuffer = await new Promise<Buffer>((resolve, reject) => {
    ...
    const pdfPromises = students.map((student) => {
      ...
      return renderToBuffer(element as any).then((buf) => {
        archive.append(buf, { name: `id-card-${student.username}.pdf` });
      });
    });
  ...
  const label = class_name
    ...
    : "all-students";
  const filename = `id-cards-${label}.zip`;
